...
def Cyrus_Beck():
    ...
    for i in range(-1, len(polygon_axes)-1):
        ...
        # Если данный параметр равен нулю, то значит что прямая параллельна данному отрезку и есть 2 возможных варианта:
        # 1) Если прямая лежит внутри фигуры, то можно продолжить расчёты
        # 2) Прямая лежит снаружи фигуры, то можно не продолжать расчёты, т.к. не будет других действительных пересечений
        ...
        # Если скалярное произведение вектора нормали и вектора отрезка положительно, то вектор входит внутрь фигуры
    ...
    # TO DO: подумать насчёт формата вывода: через параметр t или сразу координатами, или вообще всё рисовать в функции
    pass 
